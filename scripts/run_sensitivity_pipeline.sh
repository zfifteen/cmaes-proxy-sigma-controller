...
CONFIG_PATH="experiments/config/sensitivity.yaml"
RUN_ID="${RUN_ID:-}"

if [[ -z "$RUN_ID" ]]; then
  RUN_ID="$(python3 - <<'PY'
from experiments.io import load_yaml_config, make_run_id, stable_config_hash
cfg = load_yaml_config('experiments/config/sensitivity.yaml')
print(make_run_id(stable_config_hash(cfg)))
PY
)"
fi

RUN_ROOT="artifacts/runs/sensitivity/${RUN_ID}"
RESULTS_DIR="${RUN_ROOT}/results"
FIG_DIR="${RUN_ROOT}/figures"

if [[ -e "$RUN_ROOT" ]]; then
  echo "[sensitivity] ERROR: run root already exists: $RUN_ROOT" >&2
  exit 1
fi
...
echo "[sensitivity] run_id=$RUN_ID"
echo "[sensitivity] run_root=$RUN_ROOT"
