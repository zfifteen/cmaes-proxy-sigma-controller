...
CONFIG_PATH="experiments/config/smoke.yaml"
RUN_ID="${RUN_ID:-}"

if [[ -z "$RUN_ID" ]]; then
  RUN_ID="$(python3 - <<'PY'
from experiments.io import load_yaml_config, make_run_id, stable_config_hash
cfg = load_yaml_config('experiments/config/smoke.yaml')
print(make_run_id(stable_config_hash(cfg)))
PY
)"
fi

RUN_ROOT="artifacts/runs/smoke/${RUN_ID}"
RESULTS_DIR="${RUN_ROOT}/results"
FIG_DIR="${RUN_ROOT}/figures"

if [[ -e "$RUN_ROOT" ]]; then
  echo "[smoke] ERROR: run root already exists: $RUN_ROOT" >&2
  exit 1
fi
